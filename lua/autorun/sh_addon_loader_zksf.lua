...
ZKSlicerFramework.VERSION_TYPE = ".WS"
...
function ZKSlicerFramework:CheckUpdates()
	http.Fetch("https://raw.githubusercontent.com/Zak2801/slicer-framework/blob/main/lua/autorun/sh_addon_loader_zksf.lua", function(contents,size) 
		...
		if ZKSlicerFramework.VERSION_GITHUB == 0 then
			...
			if  ZKSlicerFramework:GetVersion() >= ZKSlicerFramework.VERSION_GITHUB then
				print("[ZKSlicerFramework] is up to date, Version: "..ZKSlicerFramework:GetVersion())
			else
...
ZKSlicerFramework:CheckUpdates()
...
hook.Add( "InitPostEntity", "!!!zks_sf_checkupdates", function()
	timer.Simple(3, function() ZKSlicerFramework:CheckUpdates() end)
end )